...
fn solve(reader: impl std::io::BufRead) -> u64 {
    ...
    for line in reader.lines().map(Result::unwrap) {
        ...
        let mut input_final = input.clone();
        let mut sizes_final = sizes.clone();

        for _ in 0..4 {
            input_final.push('?');
            input_final.extend(&input);
            sizes_final.extend(&sizes);
        }

        let mut mem = HashMap::<(usize, usize), u64>::new();

        result += count(0, 0, &input_final, &sizes_final, &mut mem);
    }
