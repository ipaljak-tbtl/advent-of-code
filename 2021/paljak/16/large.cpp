...
struct Packet {
  ...
  long long evaluate() {
    ...
    if (type_id == 2) {
      value = subpacket[0].evaluate();
      for (auto &packet : subpacket)
        value = min(value, packet.evaluate());
    }

    if (type_id == 3) {
      value = subpacket[0].evaluate();
      for (auto &packet : subpacket)
        value = max(value, packet.evaluate());
    }

    if (type_id == 5)
      value = subpacket[0].evaluate() > subpacket[1].evaluate();
...
Packet parse(int i) {
  ...
  if (type_id == 4) {
    ...
    while (bits[i] != '0') {
      value <<= 4;
      value += (long long)bin2int(i + 1, i + 5);
      size += 5;
      i += 5;
      assert(i < n);
    }
    ...
    value += (long long)bin2int(i + 1, i + 5);
    assert(i + 5 < n);
    return Packet(version, type_id, size + 5, {}, value);
  }
  ...
  if (length_type_id == 0) {
    ...
    return Packet(version, type_id, 3 + 3 + 1 + 15 + subpacket_size, subpacket, -1);
  } else {
    ...
    return Packet(version, type_id, size, subpacket, -1);
  }
}
