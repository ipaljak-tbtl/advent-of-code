...
void fill_positions(const vector<vector<char>> &m, int i, int j, const map<char, int> &final, vector<pair<int, pair<int, int>>> &positions) {
    ...
    while(!q.empty()) {
        ...
        if (top.first > 1 && final.at(m.at(i).at(j)) != top.second) continue;
        if (top.first > 1 && m[top.first + 1][top.second] == '.') continue;
        if (top.first > 1 && top.first + 1 == i && top.second == j) continue;
        if (i == 1 && top.first == 1) continue;
        if (top.first == 2 && m[top.first + 1][top.second] != m[i][j]) continue;

        bool add = true;
        if (top.first == 1) {
            for(const auto &item : final) {
                if (item.second == top.second) add = false;
            }
        }

        // if (add) cout << i << " " << j << " " << top.first << " " << top.second << " " << distance << " " << endl;

        if (add) positions.push_back(top_all);
    }
...
map<vector<vector<char>>, vector<vector<char>>> neighs;
map<vector<vector<char>>, int> costeri;


void print_sol(const vector<vector<char>> &m) {
    if (neighs.find(m) != neighs.end()) print_sol(neighs[m]);
    cout << costeri[m] << endl;
    print(m);
}
...
int find_solution(const vector<vector<char>> &m, const map<char, int> &final) {
    ...
    while(!q.empty()) {
        ...
        for(const auto &new_state : new_states) {
            ...
            neighs[new_state.second] = state;
            costeri[new_state.second] = cost - new_state.first;
