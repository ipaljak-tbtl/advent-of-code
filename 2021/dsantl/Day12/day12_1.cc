...
int count_paths(map<string, vector<string>> &graph) {
    ...
    while(!stack.empty()) {
        ...
        if (top.next_index == graph[top.node].size()) {
            if (top.node == twice) twice = "";
            else forbidden.erase(top.node);
            continue;
        }
        ...
        if (next_one == "start") continue;
        if (forbidden.find(next_one) != forbidden.end() && twice != "") continue;
        if (forbidden.find(next_one) != forbidden.end()) twice = next_one;
