...
parseInputData :: String -> S.Set (Int, Int, Int)
parseInputData = S.fromList . map tupleToTriple . concat . map listToTupleIndex . zip [0,1..] . map (matchIndeces '#') . lines
    where tupleToTriple (x, y) = (x, y, 0)
...
countActiveSquares :: Int -> S.Set (Int, Int, Int) -> Int
...
neighbourIndices :: (Int, Int, Int) -> S.Set (Int, Int, Int)
neighbourIndices (i, j, k) = S.fromList [(x, y, z) | x <- [i-1, i, i+1],
                                                     y <- [j-1, j, j+1],
                                                     z <- [k-1, k, k+1],
                                                     (x, y, z) /= (i, j, k)]

findRemainingActiveSquares :: S.Set (Int, Int, Int) -> S.Set (Int, Int, Int)
...
findNewActiveSquares :: S.Set (Int, Int, Int) -> S.Set (Int, Int, Int)
